...
static int
umcoap_send(umcoap_ctx_t *ctx, const char *s_uri, const void *data, size_t sz)
{
    // context check
    if (ctx == NULL) {
        return 1;
                     ...
                     const struct json_object *j_conn)
{
    if (type == UMCOAP_CLIENT) {
        ...
        // start server thread
        pthread_create(&c->server.th, NULL, &thread_srvr, c);
